...
bool str3_eq(const uint8_t *l, size_t l_length, const uint8_t *r, size_t r_length) {
	if (l_length != r_length) return false;
	return !strncmp((char *) l, (char *) r, l_length);
}
...
int str3_copy(uint8_t *destination, const uint8_t *source) {
	strcpy((char *) destination, (char *) source);
	return ERR_OK;
}

size_t str3_length(const uint8_t *string) {
...
int str3_copyMalloc(uint8_t **destination, const uint8_t *source) {
	int e = ERR_CRITICAL;
	e = str3_realloc(destination, str3_length(source));
	if (e) return e;
	str3_copy(*destination, source);
	return e;
}
